-- Update submit_ballot to support a 'full' voting stage (all 10 places at once)
...
BEGIN
  ...
  ELSIF v_stage = 'full' THEN
    v_min_rank := 1; v_max_rank := 10; v_required_count := 10;
  ELSE
  ...
  -- Validate all ranks are within the allowed range and all required are present
  IF NOT EXISTS (
